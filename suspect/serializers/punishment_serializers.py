...
class PunishmentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Punishment
        fields = '__all__'
        read_only_fields = ('id', 'issued_at', 'is_paid',
...
class PunishmentCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Punishment
        fields = ('suspect_crime', 'case', 'punishment_type', 'title',
                  'description', 'amount', 'duration_months')
    ...
    def validate(self, data):
        ...
        if data['punishment_type'] == 'imprisonment' and not data.get('duration_months'):
            raise serializers.ValidationError(
