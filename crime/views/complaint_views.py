...
from crime.models import Complaint, Case
    ...
    def get_queryset(self):
        ...
        if role_title == 'Administrator':
            ...
            return Complaint.objects.filter(
                status__in=['pending_cadet', 'rejected_by_cadet'],
                cadet=user
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if is_confirmed:
            ...
            if complaint.rejection_count >= 3:
                ...
                message = "The complaint has been marked as invalid after three rejections."
            else:
...
class ComplaintReviewByOfficerView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if is_confirmed:
            ...
            new_cadet = User.objects.filter(role__title='Cadet').exclude(
                id=complaint.cadet.id).first()
            if new_cadet:
                complaint.cadet = new_cadet
                complaint.status = 'pending_cadet'
                message = "Complaint rejected by officer and reassigned to another cadet."
            else:
                message = "Complaint rejected by officer, but no alternative cadet was found."
