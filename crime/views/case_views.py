...
from crime.models import Case, Complaint, CrimeScene
from crime.serializers import (
    CaseSerializer, CaseDetailSerializer, CaseListSerializer
)
from crime.permissions import IsDetective, IsSergeant, IsCaptain, IsChief
...
class CaseViewSet(viewsets.ModelViewSet):
    ...
    def assign_detective(self, request, pk=None):
        ...
        if not detective_id:
            return Response(
                {"error": "detective_id required"},
                status=status.HTTP_400_BAD_REQUEST
            )

        try:
            detective = User.objects.get(
                id=detective_id, role__title='Detective')
