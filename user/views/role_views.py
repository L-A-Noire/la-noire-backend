...
from user.seiralizers import RoleSerializer, RoleDetailSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Role.objects.filter(title__iexact=title).exists():
            raise ValidationError(
                {"title": "A role with this title already exists."})
...
class RoleRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if new_title and Role.objects.filter(title__iexact=new_title).exclude(pk=instance.pk).exists():
            raise ValidationError(
                {"title": "A role with this title already exists."})
